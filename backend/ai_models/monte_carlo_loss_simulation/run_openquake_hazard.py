...
if __name__ == "__main__":
    ...
    # Ensure we are running from the backend directory so paths work
    sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    
    print("Loading commune data...")
    
    # Adjust this path if your commune file is named differently
    # Based on RASED architecture, it should be in backend/data/commune_zones.csv
    COMMUNE_FILE = "../dataset/data/commune_zones.csv"
    
    try:
        df = pd.read_csv(COMMUNE_FILE)
    except FileNotFoundError:
        print(f"ERROR: Could not find {COMMUNE_FILE}.")
        print("Please make sure you are running this from the 'backend/' directory.")
        sys.exit(1)

    # Ensure the columns match what the script expects
    # Rename your actual CSV columns to match the script's expectations if needed
    col_mapping = {
        "code_wilaya": "wilaya_code",
        "commune": "commune_name",
        "zone_sismique": "zone_sismique",
        "latitude": "lat",  # adjust based on your actual csv column name
        "longitude": "lon"  # adjust based on your actual csv column name
    }
    
    # Only rename columns that actually exist in the CSV to avoid KeyErrors
    for csv_col, script_col in col_mapping.items():
        if csv_col in df.columns and script_col not in df.columns:
            df.rename(columns={csv_col: script_col}, inplace=True)
